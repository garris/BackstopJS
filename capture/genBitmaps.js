...
function capturePageSelectors(url, scenarios, viewports, bitmaps_reference, bitmaps_test, isReference) {
    ...
    casper.each(scenarios, function(casper, scenario, scenario_index) {
        ...
        casper.each(viewports, function(casper, vp, viewport_index) {
            ...
            if (onBeforeScript) {
                var func;
                if (_.isFunction(onBeforeScript)) {
                    func = onBeforeScript;
                } else {
                    func = require(getScriptPath(onBeforeScript));
                }
                func.call(casper, casper, scenario, vp, isReference);
            }
            ...
            casper.then(function() {
                ...
                if (onReadyScript) {
                    var func;
                    if (_.isFunction(onReadyScript)) {
                        func = onReadyScript;
                    } else {
                        func = require(getScriptPath(onReadyScript));
                    }
                    func.call(casper, casper, scenario, vp, isReference)
                }
            });
