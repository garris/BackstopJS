...
module.exports = function (config) {
  ...
  console.log("starts comparing images", config);
  ...
  return tests = map(compareConfig.testPairs, function (pair) {
    ...
    if(pair.reference.indexOf(config.projectPath) !== -1) {
      var referencePath = pair.reference;
      var testPath = pair.test;
    } else {
      var referencePath = path.join(config.projectPath, pair.reference);
      var testPath = path.join(config.projectPath, pair.test);
    }
    console.log("paths", testPath, referencePath);
